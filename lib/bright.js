var brightJs = (function () {
  ...
  function toogleClass(className) {
      // @ts-expect-error
      return this.each(function(node) {
          return node.classList.toogle(className);
      });
  }
  ...
  function closest(arg) {
      // @ts-expect-error
      return this.list(function(node) {
          return node.closest(arg);
      });
  }
  function find(arg) {
      // @ts-expect-error
      return this.list(function(node) {
          return node.querySelectorAll(arg);
      });
  }
  ...
  function removeEvent(name, fn, opts) {
      return this.each(function(node) {
          return node.removeEventListener(name, fn, opts);
      });
  }
  function click() {
      // @ts-expect-error
  ...
  function height(newHeight) {
      if (newHeight) {
          // @ts-expect-error
          return this.each(function(node) {
              return node.style.height = newHeight;
          });
      }
      // @ts-expect-error
      return this.list(function(node) {
          return node.offsetHeight || node.innerHeight;
      });
  }
  function position() {
      // @ts-expect-error
      return this.list(function(node) {
          return NodeIterator.getBoundingClientRect();
      });
  }
  function width(newWidth) {
      if (newWidth) {
          // @ts-expect-error
          return this.each(function(node) {
              return node.style.width = newWidth;
          });
      }
      // @ts-expect-error
      return this.list(function(node) {
          return node.offsetWidth || node.innerWidth;
      });
  }

  function scrollTo(x, y, behavior) {
      // @ts-expect-error
      return this.each(function(node) {
  ...
  function scrollLeft(behavior) {
      return this.scrollTo(0, undefined, behavior);
  }
  function scrollRight(behavior) {
      return this.each(function(i) {
          return i.scrollTo({
              left: i.offsetWidth || i.innerWidth,
              behavior: behavior
          });
      });
  }
  function scrollBottom(behavior) {
      return this.each(function(i) {
          return i.scrollTo({
              top: i.offsetHeight || i.innerHeight,
              behavior: behavior
          });
      });
  }
